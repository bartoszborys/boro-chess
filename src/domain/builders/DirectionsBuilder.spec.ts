...
describe("DirectionsBuilder", () => {
  describe("default deltas", () => {
    it("adds top as positive Y", () => {
      expect(DirectionsBuilder.create().addTopDirection().build()).toEqual([
    ...
    it("adds bottom-left as negative X and negative Y", () => {
      expect(
        DirectionsBuilder.create().addBottomLeftDirection().build(),
      ).toEqual([new Direction({ deltaX: -1, deltaY: -1 })]);
    });

    it("adds bottom-right as positive X and negative Y", () => {
      expect(
        DirectionsBuilder.create().addBottomRightDirection().build(),
      ).toEqual([new Direction({ deltaX: 1, deltaY: -1 })]);
    });

    it("adds left as negative X", () => {
      expect(DirectionsBuilder.create().addLeftDirection().build()).toEqual([
        new Direction({ deltaX: -1, deltaY: 0 }),
      ]);
    });

    it("adds right as positive X", () => {
      expect(DirectionsBuilder.create().addRightDirection().build()).toEqual([
        new Direction({ deltaX: 1, deltaY: 0 }),
      ]);
    });
  });
  ...
  describe("addCustomDirection", () => {
    it("adds a direction with arbitrary deltas", () => {
      expect(
        DirectionsBuilder.create().addCustomDirection(20, 20).build(),
      ).toEqual([new Direction({ deltaX: 20, deltaY: 20 })]);
    });
  });
});

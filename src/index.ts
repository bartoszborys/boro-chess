...
import { boardFactory, moveAnalyzer, newGameUseCase, playerFigureMoveUseCase, renderBoard } from "./bootstrap";
...
async function main() {
    ...
    const selectFigureToMove = new SelectFigureToMoveUseCase(boardFactory, moveAnalyzer);

    try {
        for (let i = 0; i < 10; i++) {
            const key = await rl.question(`Select figure [${i + 1}/10] (x-y): `);
            const coordinates = coordinatesFromKey(key);
            const availableMoves = selectFigureToMove.execute(coordinates);

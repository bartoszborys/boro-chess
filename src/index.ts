...
function coordinatesFromKey(key: string): Coordinates {
  ...
  if (!Number.isInteger(x) || !Number.isInteger(y)) {
    throw new Error(
      `Invalid coordinates key: "${key}". Expected format: x-y (e.g. 8-1)`,
    );
  }
...
function getExampleMoves(): Movement[] {
  const horseMoveTo = new Movement(
    new Coordinates(7, 1),
    new Coordinates(6, 3),
  );
  const horseMoveFrom = new Movement(
    new Coordinates(6, 3),
    new Coordinates(7, 1),
  );
  const exampleMoves = [
...
async function main() {
  ...
  const selectFigureToMove = new SelectFigureToMoveUseCase(
    boardFactory,
    moveAnalyzer,
  );
